...
# python3 may exist as a Windows Store stub that doesn't work — test with --version
PYTHON="python3"
if ! python3 --version &>/dev/null 2>&1; then
    PYTHON="python"
fi

# Force UTF-8 encoding on Windows (avoids GBK codec errors)
export PYTHONUTF8=1
...
get_actions_for_role() {
    ...
    $PYTHON - "$FLOW_YAML" "$role_name" << 'PYEOF'
import sys
try:
    import yaml
except ImportError:
