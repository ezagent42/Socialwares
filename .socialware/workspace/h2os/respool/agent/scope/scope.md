...
- 资源推荐：根据工作负载和预算推荐最优资源
- 分配管理：创建、查看、释放资源分配，支持批量分配
- 费用管理：分配费用估算、费用报表、账单查看
- 容量监控：资源池余量预警
- 租约提醒：分配到期提醒、pending 审批超时提醒
- 审批管理：pending 分配审批（admin）
- 结算管理：released 分配结算标记（admin）
- 争议处理：结算争议发起和处理
- 配额管理：消费者分配配额的查看和设置（admin）
