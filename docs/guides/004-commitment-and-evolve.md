...
```yaml
# .runtime/agents/reviewer/commitment_watch.yaml (auto-generated by deploy.sh)
watch:
  - commitment: C1
    action: review_code
...
This keeps commitment invisible to the agent while providing structured data for the evolver.
...
- Commitment fulfillment using tagged log entries
...
```
Run app → hooks tag conversation data → evolver analyzes
→ fulfillment rates per commitment (from tagged logs)
...
| Sees commitment | No (not in SOUL.md) | Yes (reads commitment.yaml + tagged logs) |
