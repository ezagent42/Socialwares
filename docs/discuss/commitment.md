...
2. Does NOT add commitment to non-evolver SOUL.md — only evolver sees commitment

### 3. Data Capture (hooks capture conversation data)

`log_prompt.sh` (UserPromptSubmit) + `log_tool.sh` (PreToolUse) hooks capture all prompts and tool calls to `.runtime/data/prompts/`. Hooks do NOT tag entries with commitment IDs -- they log all interactions uniformly. This is transparent to the agent — it does not know its interactions are being captured.

### 4. Evolver Evaluates (reads logs + commitment.yaml)

Evolver's `diagnose.py` reads conversation logs and for each commitment:
1. Extracts events matching the commitment's `from` and `to` actions
...
For each commitment:
  ...
  2. Evolver (LLM) checks condition (interprets natural language)
...
Hooks capture all prompts and tool calls uniformly to `.runtime/data/prompts/`. The evolver's `diagnose.py` reads these logs and matches events to commitments:

1. For each commitment, extracts events where the tool/action matches `from.action` or `to.action`
2. Collects timestamps, roles, and event details
3. Outputs structured data for the evolver (LLM) to judge conditions

This mechanism keeps commitment invisible to the agent while providing data for the evolver to analyze.
...
→ diagnose.py extracts commitment-related events
