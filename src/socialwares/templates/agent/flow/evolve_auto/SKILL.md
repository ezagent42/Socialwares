...
uv run --no-project --with pyyaml agent/flow/evolve_auto/scripts/run_auto.py --tests-dir agent/flow/evolve_auto/conversation_tests --adapter claude
```
...
WORKSPACE_ROOT=$(cat .workspace_root)
cd "$WORKSPACE_ROOT"
uv run --no-project --with pyyaml agent/flow/evolve_auto/scripts/run_auto.py \
